...
export default class Messaging {
  ...
  async webhooksList (a: KV): Promise<KV> {
    const {
      channelID,
      userID,
    } = (a as KV) || {}
  ...
  webhooksListEndpoint (): string {
    return '/webhooks/'
  }

  // Create webhook
  async webhooksCreate (a: KV): Promise<KV> {
    const {
      channelID,
      kind,
      userID,
    ...
    if (!channelID) {
      console.error('webhooksCreate failed, field channelID is empty', a)
      throw Error('field channelID is empty')
    }
    if (!kind) {
      console.error('webhooksCreate failed, field kind is empty', a)
      throw Error('field kind is empty')
    }
    if (!userID) {
      console.error('webhooksCreate failed, field userID is empty', a)
      throw Error('field userID is empty')
    }
    const cfg: AxiosRequestConfig = {
      method: 'post',
  ...
  webhooksCreateEndpoint (): string {
    return '/webhooks/'
  }

  // Attach file to channel
  async webhooksUpdate (a: KV): Promise<KV> {
    const {
      webhookID,
      channelID,
      kind,
    ...
    if (!webhookID) {
      console.error('webhooksUpdate failed, field webhookID is empty', a)
      throw Error('field webhookID is empty')
    }
    if (!channelID) {
      console.error('webhooksUpdate failed, field channelID is empty', a)
      throw Error('field channelID is empty')
    }
    if (!kind) {
      console.error('webhooksUpdate failed, field kind is empty', a)
      throw Error('field kind is empty')
    }
    if (!userID) {
      console.error('webhooksUpdate failed, field userID is empty', a)
      throw Error('field userID is empty')
    }
    const cfg: AxiosRequestConfig = {
      method: 'post',
  ...
  webhooksUpdateEndpoint (a: KV): string {
    const {
      webhookID,
    } = a || {}
  ...
  // Get webhook details
  async webhooksGet (a: KV): Promise<KV> {
    const {
      webhookID,
    } = (a as KV) || {}
    if (!webhookID) {
      console.error('webhooksGet failed, field webhookID is empty', a)
      throw Error('field webhookID is empty')
    }
    const cfg: AxiosRequestConfig = {
      method: 'get',
  ...
  webhooksGetEndpoint (a: KV): string {
    const {
      webhookID,
    } = a || {}
    return `/webhooks/${webhookID}`
  ...
  // Delete webhook
  async webhooksDelete (a: KV): Promise<KV> {
    const {
      webhookID,
    } = (a as KV) || {}
    if (!webhookID) {
      console.error('webhooksDelete failed, field webhookID is empty', a)
      throw Error('field webhookID is empty')
    }
    const cfg: AxiosRequestConfig = {
      method: 'delete',
  ...
  webhooksDeleteEndpoint (a: KV): string {
    const {
      webhookID,
    } = a || {}
    return `/webhooks/${webhookID}`
  ...
  // Delete webhook
  async webhooksPublicDelete (a: KV): Promise<KV> {
    const {
      webhookID,
      webhookToken,
    } = (a as KV) || {}
    if (!webhookID) {
      console.error('webhooksPublicDelete failed, field webhookID is empty', a)
      throw Error('field webhookID is empty')
    }
    if (!webhookToken) {
      console.error('webhooksPublicDelete failed, field webhookToken is empty', a)
  ...
  webhooksPublicDeleteEndpoint (a: KV): string {
    const {
      webhookID,
      webhookToken,
    } = a || {}
  ...
  async webhooksPublicCreate (a: KV): Promise<KV> {
    const {
      webhookID,
      webhookToken,
      username,
    ...
    if (!webhookID) {
      console.error('webhooksPublicCreate failed, field webhookID is empty', a)
      throw Error('field webhookID is empty')
    }
    if (!webhookToken) {
      console.error('webhooksPublicCreate failed, field webhookToken is empty', a)
      throw Error('field webhookToken is empty')
    }
    if (!content) {
      console.error('webhooksPublicCreate failed, field content is empty', a)
  ...
  webhooksPublicCreateEndpoint (a: KV): string {
    const {
      webhookID,
      webhookToken,
    } = a || {}
